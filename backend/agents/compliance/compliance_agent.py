        ...
        for tool_info in available_tools:
            ...
            def make_tool_func(name):
                def tool_func(**kwargs):
                    # Provide goal fallback to avoid missing required params (e.g., query).
                    goal = kwargs.pop("goal", "") if isinstance(kwargs, dict) else ""
                    return tool_registry.call_tool(name, goal=goal, **kwargs)
                return tool_func
