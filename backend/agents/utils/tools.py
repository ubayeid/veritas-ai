...
class ToolRegistry:
    ...
    def _generate_compliance_report(self, report_type: str = 'full', include_recommendations: bool = True) -> Dict[str, Any]:
        ...
        if not self.hybrid_engine:
            raise ValueError("Hybrid engine required for report generation")
        
        # Generate comprehensive compliance report
        query = "Generate a comprehensive compliance report"
        if report_type == 'gaps':
            query = "Find all compliance gaps and generate a gap analysis report"
        elif report_type == 'coverage':
            query = "Analyze GDPR coverage and generate a coverage report"
