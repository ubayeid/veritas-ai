...
```bash
python -c "from backend.building_database.neo4j.neo4j_connection import Neo4jConnection; Neo4jConnection().verify_connectivity()"
```
...
python backend/building_database/neo4j/build_knowledge_graph.py --clear
```
...
└── data/
    ...
    │
    └── processed/
        ├── vector/                    # Processed: Embeddings JSON
        │   ├── company/
        │   ├── standards/
        │   └── aiid/
        └── graph/                     # Processed: Graph JSON
            ├── company_graph.json
            ├── gdpr_graph.json
            └── aiid_graph.json
```
...
with Neo4jConnection() as conn:
    ...
    builder.build_from_json("backend/data_processing/processed/graph/gdpr_graph.json")
...
with Neo4jConnection() as conn:
    ...
    # Option 1: Load from JSON (recommended)
    builder.build_from_json("backend/data_processing/processed/graph/company_graph.json")
...
with Neo4jConnection() as conn:
    ...
    # Option 1: Load from JSON (recommended)
    builder.build_from_json("backend/data_processing/processed/graph/aiid_graph.json")
...
```python
from backend.building_database.neo4j import Neo4jConnection
from backend.searching.neo4j_queries import KnowledgeGraphQueries
...
See `backend/searching/neo4j_queries.py` for a full collection of query methods, or run:

```bash
python backend/searching/neo4j_queries.py
```

Or use the example script:

```bash
python backend/building_database/neo4j/example_usage.py --query
```
...
The `add_embeddings.py` script connects FAISS embeddings to Neo4j nodes:
...
1. **Load JSON File**: Reads embeddings from `backend/data_processing/processed/vector/`
...
3. Test connection: `python -c "from backend.building_database.neo4j.neo4j_connection import Neo4jConnection; Neo4jConnection().verify_connectivity()"`
...
- **`facebook_documents_builder.py`**: Creates Document/Clause nodes (processes 3 PDFs → 3 Document nodes)
- **`aiid_incidents_builder.py`**: Creates Incident nodes
- **`add_embeddings.py`**: Connects FAISS embeddings to Neo4j nodes
- **`queries.py`**: Collection of useful query methods
