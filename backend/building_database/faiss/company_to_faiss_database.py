"""
Script to build a FAISS vector database from COMPANY embedding JSON files.
Specifically converts company document embeddings to a searchable FAISS index.

This script:
1. Reads all *_embeddings.json files from backend/data_processing/processed/vector/company/
2. Extracts embeddings and metadata from each file
3. Combines all embeddings into a single FAISS index
4. Saves the index, metadata, and summary to backend/building_database/faiss/company/
...
if __name__ == "__main__":
    # Get project root directory (assuming script is in backend/building_database/faiss/)
    script_dir = Path(__file__).parent
    project_root = script_dir.parent.parent.parent
    
    # Company embeddings directory (input)
    EMBEDDINGS_DIR = project_root / "backend" / "data_processing" / "processed" / "vector" / "company"
    
    # FAISS output directory (output) - explicitly set to company subdirectory
    OUTPUT_DIR = project_root / "backend" / "building_database" / "faiss" / "company"
    
    # Index name
    INDEX_NAME = "company_faiss_index"
    
    print("="*60)
    print("COMPANY EMBEDDINGS TO FAISS DATABASE")
    print("="*60)
    print(f"Input directory: {EMBEDDINGS_DIR}")
    ...
    try:
        build_cosine_index(str(EMBEDDINGS_DIR), str(OUTPUT_DIR), INDEX_NAME)
        print("\n✓ Successfully built company FAISS database!")
    except Exception as e:
        print(f"\n✗ Error building FAISS database: {e}")
        raise
    
    # Optional: Uncomment below to also build L2 index
    # print("\nBuilding FAISS index with L2 distance...")
    # build_faiss_index(str(EMBEDDINGS_DIR), str(OUTPUT_DIR), f"{INDEX_NAME}_l2")
